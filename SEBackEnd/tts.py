...
async def get_text():
    ...
    try:
        ...
        output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'src')  # 当前绝对路径
        if not os.path.isdir(output_dir):
            os.makedirs(output_dir)
        file_prefix = datetime.now().strftime('%H_%M_%S')
        print(file_prefix)
        output_filename = os.path.join(output_dir, f"{file_prefix}.mp4")
        print(output_filename)
        await read_text(voice, text, output_filename)

        response['response']['data'] = f"{file_prefix}.mp4"
